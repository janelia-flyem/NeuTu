...
class PolicyMax(BaseSeedPolicy):
  ...
  def _init_coords(self):
    ...
    self.coords=tuple(zip(*[idx.flat[sort_idx] for idx in idxs]))
    
    
    
class PolicyGridMax(BaseSeedPolicy):
    def _init_coords(self):
        coords=[]
        img=self.canvas.image.astype(np.float32)
        d,h,w=img.shape
        threshold=float(self.kwargs['threshold'])
        for k in range(0,d-10,10):
            for j in range(0,h-20,20):
                for i in range(0,w-20,20):
                    patch=img[k:k+10,j:j+20,i:i+20]
                    if patch.max()>threshold:
                        idx=patch.argmax()
                        z = idx//400
                        y = (idx-z*400)//w
                        x = idx-z*400-y*10
                        coords.append((z+k,y+j,x+i))
...
class PolicyUserDefined(BaseSeedPolicy):
  """Seeds are defined bu user"""
  
  def _init_coords(self):
    coords=[]
    if 'seed_file' in self.kwargs:
        with open(self.kwargs['seed_file']) as f:
            for line in f.readlines():
                coord=tuple(map(int,line.split()))
                coords.append(coord)
            self.coords=np.array(coords)
    elif 'seeds' in self.kwargs:
        seeds=json.loads(self.kwargs['seeds'])
        self.coords=np.array(seeds)
