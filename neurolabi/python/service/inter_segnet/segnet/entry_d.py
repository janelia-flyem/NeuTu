...
MODEL = './segnet/data/modeld/model.ckpt-106000'
SHAPE = (12,12,12)
BATCH_SIZE = 50
SEG_THRESHOLD = 0.5
MOVE_THRESHOLD = 0.5
    ...
    for d in (z,z-DELTA,z+DELTA):
        for h in (y,y-DELTA,y+DELTA):
            for w in (x,x-DELTA,x+DELTA):
                #if np.all(pred[d-4:d+4,h-4:h+4,w-4:w+4] >= MOVE_THRESHOLD) and (d//DELTA,h//DELTA,w//DELTA) not in visited:
                if np.all(pred[d-1:d+1,h-1:h+1,w-1:w+1] >= MOVE_THRESHOLD) and (d//DELTA,h//DELTA,w//DELTA) not in visited and mask[d,h,w] == 1:
                    seeds.append((d,h,w))
