...
ZKeyEventSwcMapper::ZKeyEventSwcMapper(neutu::Document::ETag tag) :
  m_docTag(tag)
{
  initKeyMap();
}
...
void ZKeyEventSwcMapper::setTag(neutu::Document::ETag tag)
{
  m_docTag = tag;
  updateKeyMap();
}
...
void ZKeyEventSwcMapper::updateKeyMap()
{
  if (m_docTag == neutu::Document::ETag::FLYEM_SPLIT ||
      m_docTag == neutu::Document::ETag::FLYEM_PROOFREAD) {
