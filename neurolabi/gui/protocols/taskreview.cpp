...
namespace {
  ...
  static const QString KEY_BODY_ID_A = "body ID A";
  static const QString KEY_BODY_ID_B = "body ID B";

  // An alternative input format takes supervoxel IDs that are mapped to body IDs.
  ...
  // All the TaskReview instances loaded from one JSON file need certain changes
...
TaskReview::TaskReview(QJsonObject json, ZFlyEmBody3dDoc* bodyDoc)
{
...
bool TaskReview::skip(QString &reason)
{
  ...
  SetBodiesResult setBodiesResult = SetBodiesResult::SUCCEEDED;
  if (m_bodyIds.empty()) {
  ...
  LINFO() << "TaskReview::skip() HEAD took" << timer.elapsed() << "ms to decide to"
...
void TaskReview::beforeDone()
{
  restoreOverallSettings(m_bodyDoc);
}

QWidget *TaskReview::getTaskWidget()
{
...
QMenu *TaskReview::getTaskMenu()
{
  return m_menu;
}

bool TaskReview::usePrefetching()
{
  if (m_bodyIds.empty()) {
    setBodiesFromSuperVoxels();
  }
...
const std::set<uint64_t>& TaskReview::getBodyIds() const
{
  return m_bodyIds;
}
...
bool TaskReview::onLoadedZoomToFitSmaller() const
{
  return true;
}

QString TaskReview::labelForDoMajorButton() const
{
  return "";
}

QString TaskReview::valueResultDoMajor() const
{
  return "";
}
...
void TaskReview::onCycleAnswer()
{
  if (m_dontButton->isChecked()) {
    m_doButton->setChecked(true);
  } else if (m_doButton->isChecked()) {
    QRadioButton *next = includeExtendedResults() ? m_doMajorButton : m_dontButton;
    next->setChecked(true);
  } else if (m_doMajorButton && m_doMajorButton->isChecked()) {
    m_dontButton->setChecked(true);
  }
}

void TaskReview::onButtonToggled()
{
  updateColors();
...
void TaskReview::onSelectCurrentBody()
{
...
void TaskReview::onNextBodyToSelect()
{
  incrBodyToSelect();
...
void TaskReview::onPrevBodyToSelect()
{
  decrBodyToSelect();
  while (bodyToSelectIsFilteredOut()) {
    decrBodyToSelect();
...
void TaskReview::onShowSupervoxelsChanged(int state)
{
  bool show = (state != Qt::Unchecked);
  applyColorMode(show);
}

void TaskReview::onShowMajorChanged(int state)
{
  bool show = (state != Qt::Unchecked);
  if (!show) {
    while (bodyToSelectIsFilteredOut()) {
...
void TaskReview::onShowMinorChanged(int state)
{
  bool show = (state != Qt::Unchecked);
  if (!show) {
    while (bodyToSelectIsFilteredOut()) {
...
void TaskReview::onToggleShowSupervoxels()
{
...
void TaskReview::zoomOutToShowAll()
{
  zoomToFitMeshes(false);
}

void TaskReview::onHideSelected()
{
...
void TaskReview::onToggleIsolation()
{
  m_hiddenIds.clear();
...
TaskReview::SetBodiesResult TaskReview::setBodiesFromSuperVoxels()
{
  ...
  if (statusCode == 200) {
    ...
    LWARN() << "TaskReview::setBodiesFromSuperVoxels() failed for" << targetString()
  ...
  LINFO() << "TaskReview: checking sizes for" << targetString()
...
void TaskReview::buildTaskWidget()
{
  m_widget = new QWidget();

  m_dontButton = new QRadioButton(labelForDontButton(), m_widget);
  m_dontButton->setChecked(true);
  connect(m_dontButton, SIGNAL(toggled(bool)), this, SLOT(onButtonToggled()));

  m_doButton = new QRadioButton(labelForDoButton(), m_widget);
  connect(m_doButton, SIGNAL(toggled(bool)), this, SLOT(onButtonToggled()));

  if (includeExtendedResults()) {
    m_doMajorButton = new QRadioButton(labelForDoMajorButton(), m_widget);
    connect(m_doMajorButton, SIGNAL(toggled(bool)), this, SLOT(onButtonToggled()));
    ...
    m_doMajorButton = nullptr;
    m_processFurtherButton = nullptr;
    m_irrelevantButton = nullptr;
  }
  ...
  QHBoxLayout *showMajorMinorLayout = nullptr;
  if (includeExtendedResults()) {
    ...
    showMajorMinorLayout = new QHBoxLayout;
  ...
  m_menu = new QMenu(actionString(), m_widget);
...
void TaskReview::updateColors()
{
  if (Z3DMeshFilter *filter = getMeshFilter(m_bodyDoc)) {
    ...
    for (uint64_t id : m_majorBodyIds) {
      bool doing = m_doButton->isChecked() || (m_doMajorButton && m_doMajorButton->isChecked());
      m_bodyIdToColorIndex[id] = doing ? 1 : index;
      index++;
    ...
    for (uint64_t id : m_bodyIds) {
      if (m_majorBodyIds.find(id) == m_majorBodyIds.end()) {
        m_bodyIdToColorIndex[id] = m_doButton->isChecked() ? 1 : index;
        index++;
        if (index == INDEX_COLORS.size()) {
          index = 1;
        }
...
bool TaskReview::bodyToSelectIsFilteredOut() const
{
...
void TaskReview::incrBodyToSelect()
{
  m_bodyToSelect++;
  m_bodyToSelectIndex++;
...
void TaskReview::decrBodyToSelect()
{
...
void TaskReview::updateSelectCurrentBodyButton()
{
...
void TaskReview::selectBodies(const std::set<uint64_t> &bodies, bool select)
{
  if (select) {
...
void TaskReview::updateVisibility()
{
  QList<ZMesh*> meshes = ZStackDocProxy::GetGeneralMeshList(m_bodyDoc);
...
void TaskReview::zoomToFitMeshes(bool onlySmaller)
{
  QList<ZMesh*> meshes = ZStackDocProxy::GetGeneralMeshList(m_bodyDoc);
...
void TaskReview::displayWarning(const QString &title, const QString &text,
...
std::string TaskReview::outputKey() const
{
  ...
  bool loadIds(QJsonObject json, QString key, std::vector<uint64_t> &destination)
  {
    if (json.contains(key)) {
      ...
      if (value.isArray()) {
        ...
        uint64_t id = uint64_t(value.toDouble());
        destination.push_back(id);
      }
      return true;
    } else {
...
void TaskReview::writeOutput()
{
  ZDvidReader reader;
  reader.setVerbose(false);
  if (!reader.open(m_bodyDoc->getDvidTarget())) {
    LERROR() << "TaskReview::writeOutput() could not open DVID target for reading";
    return;
  }

  ZDvidWriter writer;
  if (!writer.open(m_bodyDoc->getDvidTarget())) {
    LERROR() << "TaskReview::writeOutput() could not open DVID target for writing";
    return;
  }
  ...
  if (!reader.hasData(instance)) {
    LERROR() << "TaskReview::writeOutput() could not create DVID instance \"" << instance << "\"";
    return;
  }
  ...
  switch (m_skip) {
    ...
    case Skip::SKIPPED_SIZES:
      ...
      break;
    default:
      if (m_doButton->isChecked()) {
        json[KEY_RESULT] = QJsonValue(valueResultDo());
        m_lastSavedButton = m_doButton;
      } else if (m_doMajorButton && m_doMajorButton->isChecked()) {
        json[KEY_RESULT] = QJsonValue(valueResultDoMajor());
        m_lastSavedButton = m_doMajorButton;
      } else if (m_dontButton->isChecked()) {
        json[KEY_RESULT] = QJsonValue(valueResultDont());
        m_lastSavedButton = m_dontButton;
      } else if (m_processFurtherButton && m_processFurtherButton->isChecked()) {
        json[KEY_RESULT] = QJsonValue(VALUE_RESULT_PROCESS_FURTHER);
        m_lastSavedButton = m_processFurtherButton;
      } else if (m_irrelevantButton && m_irrelevantButton->isChecked()) {
  ...
  if (!m_bodyIdsA.empty() && !m_bodyIdsB.empty()) {
    saveIds(json, KEY_BODY_ID_A, m_bodyIdsA);
    saveIds(json, KEY_BODY_ID_B, m_bodyIdsB);
  } else {
    saveIds(json, KEY_SUPERVOXEL_IDS_A, m_superVoxelIdsA);
    saveIds(json, KEY_SUPERVOXEL_IDS_B, m_superVoxelIdsB);

    std::vector<uint64_t> majorSuperVoxelIds(m_majorSuperVoxelIds.begin(), m_majorSuperVoxelIds.end());
    saveIds(json, KEY_SUPERVOXEL_IDS_OF_INTEREST, majorSuperVoxelIds);
...
QString TaskReview::readResult() const
{
  ZDvidReader reader;
  reader.setVerbose(false);
  if (reader.open(m_bodyDoc->getDvidTarget())) {
...
void TaskReview::restoreResult(QString result)
{
  if (result == valueResultDo()) {
    m_doButton->setChecked(true);
    m_lastSavedButton = m_doButton;
  } else if (result == valueResultDoMajor()) {
    if (m_doMajorButton) {
      m_doMajorButton->setChecked(true);
      m_lastSavedButton = m_doMajorButton;
    }
  } else if (result == valueResultDont()) {
    m_dontButton->setChecked(true);
    m_lastSavedButton = m_dontButton;
  } else if (result == VALUE_RESULT_PROCESS_FURTHER) {
    if (m_processFurtherButton) {
      m_processFurtherButton->setChecked(true);
      m_lastSavedButton = m_processFurtherButton;
    }
  } else if (result == VALUE_RESULT_IRRELEVANT) {
    if (m_irrelevantButton) {
...
bool TaskReview::loadSpecific(QJsonObject json)
{
  ...
  // One form of input explicitly lists body IDs.

  bool loadedBodyA = loadIds(json, KEY_BODY_ID_A, m_bodyIdsA);
  bool loadedBodyB = loadIds(json, KEY_BODY_ID_B, m_bodyIdsB);
  if (loadedBodyA && loadedBodyB) {
    m_bodyIds.insert(m_bodyIdsA.begin(), m_bodyIdsA.end());
    m_bodyIds.insert(m_bodyIdsB.begin(), m_bodyIdsB.end());
  ...
  // Another form of input lists supervoxel IDs, from which body IDs are derived.
...
QJsonObject TaskReview::addToJson(QJsonObject taskJson)
{
  ...
  if (!m_bodyIdsA.empty() && !m_bodyIdsB.empty()) {
    ...
    saveIds(taskJson, KEY_BODY_ID_B, m_bodyIdsB);
  } else {
...
void TaskReview::onLoaded()
{
  LINFO() << "TaskReview: build version" << getBuildVersion() << ".";
  ...
  bool onlySmaller = (onLoadedZoomToFitSmaller() && !m_superVoxelPointsA.empty());
  zoomToFitMeshes(onlySmaller);
}

bool TaskReview::allowCompletion()
{
  bool allow = true;
...
ProtocolTaskConfig TaskReview::getTaskConfig() const
{
