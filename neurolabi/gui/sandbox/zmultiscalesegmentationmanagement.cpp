...
void ZMultiscaleSegmentationWindow::onSegment(){
  ...
  shared_ptr<ZStack> stack = nullptr;
  if(m_selected_id == m_seg_tree->getRootID()){
    stack = shared_ptr<ZStack>(m_stack->clone());
  } else {
    stack = shared_ptr<ZStack>(m_stack->makeCrop(m_seg_tree->getBoundBox(m_selected_id)));
    m_seg_tree->maskStack(m_selected_id, stack.get());
  }
